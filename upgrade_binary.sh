...
function final_message() {
    ...
    if [ "$SCRIPT_STATUS" == "SUCCESS" ]; then
        echo -e "🎉 ${C_GREEN}SUCCESS: Blockchain upgrade operation completed successfully.${C_RESET}"
        echo -e "   ${C_WHITE}• Service has been upgraded and restarted${C_RESET}"
        echo -e "   ${C_WHITE}• All systems are operational${C_RESET}"
...
while true; do
    ...
    if [ "$latest_block" -ge "$TARGET_BLOCK" ]; then
        ...
        echo -e "   🔄 ${C_YELLOW}Phase 3: Restarting blockchain service...${C_RESET}"
        sudo systemctl start "$SERVICE_NAME"
        if [ $? -ne 0 ]; then
